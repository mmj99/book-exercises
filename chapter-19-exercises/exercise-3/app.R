...
# This function should perform the following:
  my_server <- function(input_list, output_list) {
    output_list$my_plot <- renderPlot({
      selected_range <- input_list$price_choice # gets the users value
      filtered_data <- diamonds_sample %>%
        filter(price > selected_range[1], price < selected_range[2]) #above the min range, below the max range that the user identified
